...
public:
  ...
  void async_operation(float *h_c, const float *h_a, const float *h_b,
                       float *d_c, float *d_a, float *d_b, const int size,
                       const int bufsize);
  void print_kernel_time();

}; // Operator

void Operator::CUDART_CB Callback(cudaStream_t stream, cudaError_t status,
                                  void *userData) {
...
int main(int argc, char *argv[]) {
  ...
  Operator_with_priority *ls_operator =
      new Operator_with_priority[num_operator];
