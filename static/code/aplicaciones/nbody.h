...
enum NBodyConfig { BODYSYSTEM_POSITION, BODYSYSTEM_VELOCITY };
...
void setArray(enum NBodyConfig array,
              float4 *data);        //-> copies Tipsy file data to PBOs
...
void idle(
    void); //-> idle callback (called whenever something is not happening...)
void finalize(); //-> clears memory when window is closed

// Simulation functions
void integrateNbodySystem(
    float4 **dPos, float4 *dVel, struct cudaGraphicsResource **pgres,
    unsigned int currentRead, float deltaTime, float damping,
    unsigned int numBodies,
    int blockSize);      //-> calls kernel to update particle positions
void updateSimulation(); //-> calls integrateNBodySystem and swaps currentRead
                         //and currentWrite values to switch pbo buffers.
