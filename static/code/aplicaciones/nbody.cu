...
__global__ void integrateBodies(float4 *__restrict__ newPos,
                                float4 *__restrict__ oldPos, float4 *vel,
...
void initialize() {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    cudaGraphicsGLRegisterBuffer(&m_pGRes[i], m_pbo[i],
  ...
  checkCUDAerror();
  cudaMemcpyToSymbol(softeningSquared, &softeningSq, sizeof(float), 0,
...
void setArray(enum NBodyConfig array, float4 *data) {
  ...
  case BODYSYSTEM_VELOCITY:
    cudaMemcpy(dVel, data, numBodies * 4 * sizeof(float),
               cudaMemcpyHostToDevice);
    break;
  }
}
...
void loadTipsyFile(char *filename) {
  ...
  read_tipsy_file(positions, velocities, filename, numBodies);
...
unsigned char *createGaussianMap(int N) {
  ...
  for (int y = 0; y < N; y++, Y += Incr) {
    ...
    for (int x = 0; x < N; x++, X += Incr, i += 2, j += 4) {
      Dist =
          (float)sqrtf(X * X + Y2); // Presumably this slows things down a lot??
...
void initGL(int *argc, char **argv) {
  ...
  // It seems including this function is not trivial because of how the OpenGL
  // libraries work. So hopefully it's not too important! glxSwapIntervalSGI(0);
  ...
  glBindBuffer(GL_ARRAY_BUFFER, m_vboColor);
  glBufferData(GL_ARRAY_BUFFER, numBodies * 4 * sizeof(float), 0,
               GL_STATIC_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, 0);
}
...
void display() {
  ...
  for (int c = 0; c < 3; ++c) {
    // globals that are used to slightly slow the camera movement when
    // rotating??
  ...
  displayNBodySystem(); // -> calls the "display" function of the NBodySystem
                        // class...
                        // -> which then calls setSpriteSize, setPBO, display in
                        // the renderer.
...
void integrateNbodySystem(float4 **dPos, float4 *dVel,
                          struct cudaGraphicsResource **pgres,
  ...
  cudaGraphicsResourceGetMappedPointer((void **)&dPos[currentRead], &bytes,
                                       pgres[currentRead]);
  cudaGraphicsResourceGetMappedPointer((void **)&dPos[1 - currentRead], &bytes,
                                       pgres[1 - currentRead]);
  ...
  integrateBodies<<<numBlocks, blockSize, sharedMemSize>>>(
      dPos[1 - currentRead], dPos[currentRead], dVel, deltaTime, damping,
      numTiles);
...
// Presumably I'll need to set the timestep deltaTime as a global (0.016 is used
// in the sim...)
void updateSimulation() {

  int temp;
  // where the magic happens!
  // the m_currentRead, m_currentWrite flags (pointers?) are used to control
  // which of the two PBO buffers we're using to write data and which to display
  // the model

  // This function is within bodysystemcuda.cu, NOT the header file!
  integrateNbodySystem(dPos, dVel, m_pGRes, m_currentRead, deltaTime, damping,
                       numBodies, blockSize);
