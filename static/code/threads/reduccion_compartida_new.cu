...
__global__ void reduccion_memoria_compartida(float *data, int N) {
  ...
  if (idx < N) {
    ...
    if (idx + 7 * blockDim.x < N) {
      ...
      sdata[threadIdx.x] = a1 + a2 + a3 + a4 + a5 + a6 + a7 + a8;
    }
    __syncthreads();
...
int main() {
  ...
  reduccion_memoria_compartida<<<n_bloques / 8, n_threads,
                                 n_threads * sizeof(float)>>>(d_array, N);
