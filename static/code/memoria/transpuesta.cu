...
void transpuestaHost(float *salida, float *entrada, const int nx,
                     const int ny) {
...
__global__ void transpuestaCargarFilas(float *salida, float *entrada,
...
__global__ void transpuestaCargarColumnas(float *salida, float *entrada,
                                          const int nx, const int ny) {
  unsigned int ix = blockDim.x * blockIdx.x + threadIdx.x;
  unsigned int iy = blockDim.y * blockIdx.y + threadIdx.y;
...
int main() {
  ...
  printf("transpuestaCargarFilas: %f s <<< grid (%d,%d) block (%d,%d)>>> "
         "bandwidth "
         "efectivo %f GB\n",
         tiempo_total, grid.x, grid.y, block.x, block.y, ibnd);
  CHECK(cudaGetLastError());
  ...
  printf("transpuestaCargarColumnas: %f s <<< grid (%d,%d) block (%d,%d)>>> "
         "bandwidth "
         "efectivo %f GB\n",
         tiempo_total, grid.x, grid.y, block.x, block.y, ibnd);
  CHECK(cudaGetLastError());
