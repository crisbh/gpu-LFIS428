...
// kernel para copiar los valores de una matriz a otra, columna por columna
__global__ void copiarColumna(float *salida, float *entrada, const int nx,
                              const int ny) {
  unsigned int ix = blockDim.x * blockIdx.x + threadIdx.x;
  unsigned int iy = blockDim.y * blockIdx.y + threadIdx.y;

  if (ix < nx && iy < ny) {
    salida[ix * ny + iy] = entrada[ix * ny + iy];
  }
}
...
int main() {
  ...
  copiarColumna<<<grid, block>>>(d_C, d_A, nx, ny);
  ...
  printf(
      "copiarColumna demoró  %f s <<< grid (%d,%d) block (%d,%d)>>> bandwidth "
      "efectivo %f GB/s\n",
