...
int main(int argc, char *argv[]) {
  ...
  cudaMallocManaged((void **)&p_sample,
                    sizeof(Real) * sample_size * batch_size);
  ...
  int embed_sample[] = {0};
  int embed_freq[] = {0};
  cufftPlanMany(&plan_forward, rank, &sample_size, embed_sample, stride_sample,
                dist_sample, embed_freq, stride_freq, dist_freq, CUFFT_R2C,
                batch_size);
  cufftPlanMany(&plan_inverse, rank, &sample_size, embed_freq, stride_freq,
                dist_freq, embed_sample, stride_sample, dist_sample, CUFFT_C2R,
                batch_size);
  ...
  // print elapsed time
  std::cout << "FFT operation time for " << sample_size << " elements with "
